                ...
                if (error) {
                    ...
                    cmd.on('close', function (code) {
                        if (!code) {
                            ...
                            var pack = hasPackage(path);
                            if (
                                pack
                                && typeof( pack['scripts']) != 'undefined'
                                && typeof( pack['scripts']['postinstall']) != 'undefined'
                            ) {

                                var postInstall = pack.scripts.postinstall.split(' ');
                                var node = postInstall.splice(0,1)[0];
                                var tmpCWD = process.cwd();
                                process.chdir(path);
                                var postInstallCmd = spawn(node, postInstall);
