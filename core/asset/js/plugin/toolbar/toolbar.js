var Toolbar = function() {
    ...
    // import
    var Storage             = gina.Storage
        , bucket            = new Storage({bucket: 'gina'}) // <Bucket>
...
window.onload = function(){
    if ( typeof(window['gina']) == 'undefined' ) {
        throw new Error('`gina` dependency is missing: did you load `gina.min.js` ?');
    } else if ( typeof(window['gina']) != 'undefined' && !window['gina']['isFrameworkLoaded'] ) { // when gina not loaded thru framework context
        new Toolbar()
    }
}
