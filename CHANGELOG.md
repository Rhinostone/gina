...
* 103 Early Hints (#EH1) — `self.setEarlyHints(links)` on SuperController sends a `103 Early Hints` informational response immediately when called from a controller action. HTTP/2: `stream.additionalHeaders({ ':status': 103, 'link': ... })`; HTTP/1.1: `res.writeEarlyHints()` (Node.js 18.11+). Accepts a string or array of `Link` header values. Silent no-op when headers already sent or runtime does not support informational responses. Returns `self` for chaining. Updated `status.codes`: `103` label corrected from `"Checkpoint"` to `"Early Hints"`. 22 new unit tests covering source-structure and pure logic (test/core/controller.test.js suites 04–05).
### Fixed
