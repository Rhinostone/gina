...
* SQL annotation parser (#SQL1) — replaced the single-pass regex in the Couchbase N1QL parser (`readSource`) and the SQLite parser (`readSQL`) with a state-machine-based `stripComments()` / `extractFirstBlockComment()` in `core/connectors/sql-parser.js`. Correctly handles nested block comments (`/* outer /* inner */ */`) and `--` / `//` inside string literals. Drop-in replacement — no change to annotation format or caller API. 35 new unit tests (test/core/sql-parser.test.js).
* 103 Early Hints (#EH1) — two-level implementation: (1) `render()` automatically sends a 103 with the bundle's CSS/JS preload links (from `h2Links`) before `getAssets()` and Swig compilation in HTTP/2 production mode — zero config required; (2) `self.setEarlyHints(links)` on SuperController sends a manual 103 for any additional resources the framework cannot know about (fonts, hero images, etc.). HTTP/2: `stream.additionalHeaders({ ':status': 103 })`; HTTP/1.1: `res.writeEarlyHints()` (Node.js 18.11+). Silent no-op when headers already sent or runtime unsupported. Returns `self` for chaining. `status.codes`: `103` label corrected from `"Checkpoint"` to `"Early Hints"` (RFC 8297). 27 new unit tests (test/core/controller.test.js suites 04–06).
### Fixed
