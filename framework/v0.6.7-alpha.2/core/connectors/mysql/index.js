    ...
    var readSQL = function(entities, conn, entityName, source) {
        ...
        if (typeof entities[entityName].prototype[name] !== 'undefined') {
            ...
            if ( !Object.prototype.hasOwnProperty.call(entities[entityName].prototype, name) ) {
                console.warn('[mysql] skipping query method \'' + name + '\' (' + source + '): the name is inherited on the ' + entityName + ' prototype chain (EventEmitter, entity base API, or gina\'s Object.prototype helpers like count()) — calls would run the inherited member instead. Rename the file, e.g. \'' + name + 'Rows.sql\'.');
            }
            return;
        }
