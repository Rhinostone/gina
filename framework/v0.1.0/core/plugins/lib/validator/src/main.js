...
function ValidatorPlugin(rules, data, formId) {
                ...
                if (xhr.readyState == 4) {
                    ...
                    if( /^2/.test(xhr.status) ) {
                        ...
                        if ( typeof(xhr.responseType) != 'undefined')
                            xhr.responseType = ""; // cancel response type
    ...
    var bindForm = function($target, customRule) {
        ...
        //         if ( /(button)/i.test(event.target.tagName) && event.target.type != 'submit' ) {                    
        //             ignoredEvent = true;
        //             return false;
        //             //event.stopPropagation();
        //             //cancelEvent(event);
        //             //triggerEvent(gina, event.target, 'click', event.detail);
        //         }
                 
                
        //         if ( !ignoredEvent && typeof(event.target.id) == 'undefined' || !ignoredEvent && !event.target.getAttribute('id') ) {
