define('gina/toolbar', [ 'require', 'jquery', 'vendor/uuid','utils/merge', 'utils/collection', 'gina/storage' ], function (require) {
    ...
    function Toolbar() {
        ...
        var parseForms = function (obj, ginaObj, $html, i, $forms, len) {
            ...
            var $form = $('<div class="gina-toolbar-section" id="gina-toolbar-form-html-properties">\n' +
                            '<h2 class="gina-toolbar-section-title">'+ id +'</h2>\n' +
                            '<ul class="gina-toolbar-code '+ attrClass +'"></ul>\n' +
                        '</div>');
            ...
            for ( var a = 0, aLen = attributes.length; a < aLen; ++a ) {
                ...
                if ( /^data-gina-form-event/.test(key) ) {
                    ...
                    if (!hasEvents) {
                        ...
                        $form
                            .find('.'+ attrClass)
                            .append('<div class="gina-toolbar-section">' +
                                        '<h2 class="gina-toolbar-section-title gina-toolbar-key gina-toolbar-key-events">events</h2>' +
                                        '<ul class="gina-toolbar-code gina-toolbar-value-events">' +
                                        '</ul>' +
                                    '</div>');
                    ...
                    content =   '<li>' +
                                    '<span class="gina-toolbar-key-event">'+ key +':</span>' +
                                    '<span class="gina-toolbar-value gina-toolbar-value-event">'+ val +'</span>' +
                                '</li>';
            ...
            if ( rules ) {
                ...
                $form
                    .find('.'+ attrClass)
                    .append('<div class="gina-toolbar-section">' +
                                '<h2 class="gina-toolbar-section-title gina-toolbar-key gina-toolbar-key-rules">rule</h2>' +
                                '<ul class="gina-toolbar-value-rules gina-toolbar-code">'+ parseRules( rules, id ) +'</ul>' +
                            '</div>');
