...
(function () {
    ...
    function getPageMetrics(isXhr) {
        var m = { weight: null, transferMs: null, fcpMs: null, source: 'page' };
        try {
            ...
            if (isXhr) {
                ...
                if (xhrEntry) {
                    m.transferMs = Math.round(xhrEntry.duration);
    ...
    function renderViewContent(view, ginaView) {
        ...
        var hasBadges = engine || metrics.weight || metrics.loadMs || metrics.fcpMs;
        if (hasBadges) {
            ...
            if (metrics.weight) {
                var weightTitle = isXhr ? 'XHR response transfer size' : 'Page transfer size (document)';
                h += '<span class="bm-vbadge bm-vbadge-weight" title="' + weightTitle + '">'
                    + '<svg viewBox="0 0 16 16"><path d="M3.5 1h9l2.5 14H1zM4.8 2.5h6.4l2 11H2.8z"/></svg>'
                    + formatBytes(metrics.weight) + '</span>';
            }
            if (metrics.transferMs) {
                var transferTitle = isXhr ? 'XHR round-trip duration' : 'Document transfer time (requestStart \u2192 responseEnd)';
                var transferLabel = isXhr ? 'RTT' : 'transfer';
                h += '<span class="bm-vbadge bm-vbadge-load" title="' + transferTitle + '">'
                    ...
                    + fmtMs(metrics.transferMs) + ' ' + transferLabel + '</span>';
            }
            if (metrics.fcpMs) {
    ...
    /** Shows/hides the "Copy N" button based on current selection size. */
    function updateSelectionUI() {
        var btn = qs('#bm-log-copy-sel');
        if (!btn) return;
        if (selectedLogIds.size === 0) {
            btn.classList.add('hidden');
        } else {
            btn.classList.remove('hidden');
            btn.textContent = 'Copy ' + selectedLogIds.size;
        }
    }
    ...
    function init() {
        ...
        var _copySelBtn = qs('#bm-log-copy-sel');
        if (_copySelBtn) _copySelBtn.addEventListener('click', copySelectedLogs);
