...
async function render(userData, displayToolbar, errOptions) {
    ...
    if (
        ...
        if (extendFound && Array.isArray(extendFound)) {
            try {
                ...
                newLayoutFilename = _(localOptions.template.html +'/'+ newLayoutPath, true);
                ...
                // updating extends
                _templateContent = _templateContent.replace(layoutPath, newLayoutPath);

                // override layout path
                layoutPath = newLayoutPath;

                data.page.view.layout = layoutPath;
                layoutPath = localOptions.template.html +'/'+ layoutPath;
    ...
    try {
        ...
        // Loading from cache
        if ( typeof(cachedTemplates[ localOptions.bundle + subFolder  +'/'+ data.page.view.file ]) != 'undefined' ) {
