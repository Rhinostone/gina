...
/**
 * PostgreSQL ORM connector.
 *
 * Wires entity classes with SQL methods read from the bundle's `sql/` directory.
 * SQL files use standard PostgreSQL syntax with `$1`, `$2`, … positional placeholders.
 * Backed by the `pg` pool passed from PostgresqlConnector.onReady.
 *
 * Bundle model layout:
 * ```
 * models/
 ...
 *     sql/
 *       User/
 *         findById.sql       ← SELECT … WHERE id = $1
 *         findAll.sql
 *         insert.sql
 * ```
 *
...
function Postgresql(conn, infos) {
    ...
    var readSQL = function(entities, conn, entityName, source) {
        ...
        var trigger  = 'PG:' + entityName.toLowerCase() + '#' + name;

        // ── Helper: coerce pg result to the annotated return type ─────────────
        //
        // SELECT → result.rows is plain object[] and result.rowCount is the count
        // Write  → result.rows is [] and result.rowCount is affected row count
        //          result.command is 'INSERT', 'UPDATE', 'DELETE', etc.
        // ─────────────────────────────────────────────────────────────────────
        var coerce = function(result) {
            ...
            // Default: normalised write result
