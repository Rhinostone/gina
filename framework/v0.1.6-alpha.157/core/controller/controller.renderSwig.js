...
function writeCache(localOptions, htmlContent) {
    if (
    ...
    var cacheKey = "static:"+ local.req.url;
    ...
    if ( !cache.has(cacheKey) ) {
        ...
        if ( typeof(cachingOption.ttl) == 'undefined' ) {
            cachingOption.ttl = localOptions.conf.server.cache.ttl
        }
        var cacheObject = {
        ...
        if ( /^fs$/i.test(cachingOption.type) ) {
            ...
            var htmlFilename = _(localOptions.conf.server.cache.path +'/'+ localOptions.bundle +'/html'+ url + '.html', true);
        ...
        if (
            ...
            if ( !headersSent() ) {
                ...
                if (
                    ...
                    writeCache(localOptions, htmlContent);
                }
