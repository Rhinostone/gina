 ...
 * the publish if any path in the pack listing matches a local-tool configuration name
 * (`CLAUDE.md`, `.claude*`).
 *
 ...
 *   1  — local-tool configuration path detected OR check itself errored (fail closed)
 */
...
try {
    ...
    if (matches.length > 0) {
        console.error('[prepack] ERROR: local-tool configuration files detected in pack listing:');
    ...
    console.log('[prepack] OK: pack listing contains no local-tool configuration paths.');
    process.exit(0);
} catch (err) {
